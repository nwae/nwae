...
class IntentEngineThread(threading.Thread):
    ...
    def join(self, timeout=None):
        ...
        log.Log.critical(str(self.__class__) + ' ' + str(getframeinfo(currentframe()).lineno)
                         + ': Intent Background Thread ended..')

    def run(self):
        log.Log.critical(str(self.__class__) + ' ' + str(getframeinfo(currentframe()).lineno)
                         + ': Intent Background Thread started..')
        try:
        ...
        if self.failed_to_load or (not self.check_hashes):
            log.Log.important(str(self.__class__) + ' ' + str(getframeinfo(currentframe()).lineno)
                             + ': Intent Engine Background Thread not checking hashes, quitting thread.')
            return
