...
class SuggestMetricUnitTest:
    ...
    def __test_water(self):
        ...
        df_pokupki = pd.DataFrame({
            'client': ['a', 'a', 'a', 'b', 'b', 'c', 'c', 'd', 'd', 'e', 'e', 'f', 'f'],
            'product': ['borjomi', 'borjomi', 'karspatskaya', 'borjomi', 'karspatskaya', 'illy', 'bonaqua', 'illy', 'lavazza', 'bonaqua', 'lavazza', 'lavazza', 'xxx'],
            'quantity': [0.5, 0.5, 1, 2, 1, 1, 1, 2, 1, 2, 1, 2, 0]
        })
        unique_client_cols_list = ['client']
        unique_product_col = 'product'
        value_col = 'quantity'
        ...
        df_client_profiles, product_attributes_list = self.recommend_data_profile.convert_product_to_attributes(
            ...
            unique_human_key_columns    = unique_client_cols_list,
            unique_product_key_column   = unique_product_col,
            unique_product_value_column = value_col,
        ...
        df_mapped_product = self.recommend_metric.encode_product_attributes(
            ...
            unique_human_key_columns = unique_client_cols_list,
            unique_df_object_human_key_columns  = unique_client_cols_list,
            unique_df_object_object_key_columns = [unique_product_col],
            unique_df_object_value_column       = value_col,
        ...
        x_expected_rec_euclidean_by_client = ['lavazza', 'karspatskaya', 'borjomi', othersprd, 'illy', 'bonaqua']
        x_expected_rec_cosine           = ['lavazza', 'illy', 'bonaqua', 'karspatskaya', 'borjomi', othersprd]
        x_expected_rec_cosine_by_client = ['lavazza', 'bonaqua', othersprd, 'karspatskaya', 'illy', 'borjomi']
        y_expected_rec_euclidean =           ['bonaqua', 'lavazza', 'illy', 'borjomi', 'karspatskaya', othersprd]
        y_expected_rec_euclidean_by_client = ['bonaqua', 'illy', 'lavazza', othersprd, 'karspatskaya', 'borjomi']
        y_expected_rec_cosine           = ['bonaqua', 'lavazza', 'illy', 'karspatskaya', 'borjomi', othersprd]
        y_expected_rec_cosine_by_client = ['bonaqua', 'illy', 'lavazza', othersprd, 'karspatskaya', 'borjomi']
        z_expected_rec_euclidean =           ['borjomi', 'karspatskaya', 'lavazza', 'bonaqua', 'illy', othersprd]
        z_expected_rec_euclidean_by_client = ['borjomi', 'karspatskaya', othersprd, 'illy', 'bonaqua', 'lavazza']
        z_expected_rec_cosine           = ['karspatskaya', 'borjomi', 'lavazza', 'illy', 'bonaqua', othersprd]
        z_expected_rec_cosine_by_client = ['borjomi', 'karspatskaya', othersprd, 'illy', 'bonaqua', 'lavazza']
        ...
        metric_vec_expected = [
            [SuggestMetric.METRIC_EUCLIDEAN, none_vec, none_vec_expected_rec, None],
            [SuggestMetric.METRIC_EUCLIDEAN, x_vec, x_expected_rec_euclidean, x_expected_rec_euclidean_by_client],
            [SuggestMetric.METRIC_EUCLIDEAN, y_vec, y_expected_rec_euclidean, y_expected_rec_euclidean_by_client],
            [SuggestMetric.METRIC_EUCLIDEAN, z_vec, z_expected_rec_euclidean, z_expected_rec_euclidean_by_client],
            [SuggestMetric.METRIC_COSINE, none_vec, none_vec_expected_rec, None],
            [SuggestMetric.METRIC_COSINE, x_vec, x_expected_rec_cosine, x_expected_rec_cosine_by_client],
            [SuggestMetric.METRIC_COSINE, y_vec, y_expected_rec_cosine, y_expected_rec_cosine_by_client],
            [SuggestMetric.METRIC_COSINE, z_vec, z_expected_rec_cosine, z_expected_rec_cosine_by_client],
            [SuggestMetric.METRIC_EUCLIDEAN, vecs_all, expected_euclidean_all, None],
            [SuggestMetric.METRIC_COSINE, vecs_all, expected_cosine_all, None],
        ]
        for m_v_e in metric_vec_expected:
            metric, vec, expected_recommendations, expected_recommendations_byclient = m_v_e
            recommendations = self.recommend_metric.recommend_products(
                obj_ref_dna    = vec,
                df_product_dna = df_mapped_product,
                unique_prdname_cols = [unique_product_col],
                metric = metric,
                force_normalization = (metric == SuggestMetric.METRIC_COSINE),
                replace_purchased_product_with_nan = False,
                include_products_not_in_attributes = False,
                algorithm = SuggestMetric.ALGO_BY_CLOSEST_PRODUCT,
            )
            # Обратный формат всегда 2-мерный [[1,2,3]], [[1,2,3],[4,5,6]], ..
            ...
            # TEST USING ALGORITHM BY CLOSEST CLIENT
            # Don't do for multiclient
            if ( vec is None ) or ( len(vec.shape) > 1 ):
                continue
            how_many_humans = 2
            recommendations = self.recommend_metric.recommend_products(
                ...
                replace_purchased_product_with_nan = False,
                include_products_not_in_attributes = False,
                algorithm         = SuggestMetric.ALGO_BY_CLOSEST_CLIENT,
                how_many_humans   = how_many_humans,
                df_human_dna      = df_client_profiles,
                unique_human_cols = unique_client_cols_list,
            )
            # Обратный формат всегда 2-мерный [[1,2,3]], [[1,2,3],[4,5,6]], ..
            if ( vec is None ) or ( len(vec.shape) == 1 ):
                expected_recommendations_mod = [expected_recommendations_byclient]
            else:
                expected_recommendations_mod = expected_recommendations_byclient
            res_test = UnitTest.assert_true(
                observed     = recommendations,
                expected     = expected_recommendations_mod,
                test_comment = '"By client" recomendations metric "' + str(metric) + '" for "' + str(vec)
                               + '" ' + str(recommendations) + ' expect ' + str(expected_recommendations_mod)
            )
            self.res_final.update_bool(res_bool=res_test)
            if not res_test:
