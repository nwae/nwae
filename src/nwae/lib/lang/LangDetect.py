...
class LangDetect:
    ...
    def __detect_chars(
        ...
        for rge in ranges_to_check:
            for i in rge:
                c = text[i]
                if c in LangCharacters.UNICODE_BLOCK_HANGUL_ALL_INCLUDING_SYLLABLE:
                    lang_chars.append(self.lang_number[LangFeatures.LANG_KO])
                elif c in LangCharacters.UNICODE_BLOCK_CJK:
                    lang_chars.append(self.lang_number[LangFeatures.LANG_CN])
                elif c in LangCharacters.UNICODE_BLOCK_THAI:
                    lang_chars.append(self.lang_number[LangFeatures.LANG_TH])
                elif c in LangCharacters.UNICODE_BLOCK_LATIN_VIETNAMESE:
                    lang_chars.append(self.lang_number[LangFeatures.LANG_VN])
                elif c in LangCharacters.UNICODE_BLOCK_LATIN_BASIC:
                    lang_chars.append(self.lang_number[LangFeatures.LANG_EN])

        ser = pd.Series(lang_chars)
...
if __name__ == '__main__':
    ...
    for s in text:
        ...
        print('Took ' + str(round(timedif, 2)) + ' ms.')
        break
